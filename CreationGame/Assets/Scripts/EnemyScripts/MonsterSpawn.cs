...
public class MonsterSpawn : MonoBehaviour
{
    ...
    public List<GameObject> objectPooing = new List<GameObject>(); // 오브젝트 풀링 List
    ...
    public GameObject TakeOut()
    {
        ...
        for(int i=0; i<objectPooing.Count; i++)
        {
            if(!objectPooing[i].activeInHierarchy)
            {
                objectPooing[i].SetActive(true);
                objectPooing[i].transform.position = responePos[i].transform.position + randomPos;
                return objectPooing[i];
            }
        }
